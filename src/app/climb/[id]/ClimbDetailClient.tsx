...
interface ClimbDetailClientProps {
  ...
  initialPhotos: Array<{ id: string, image_base64: string | null }>
  initialVideos: Array<{ id: string, url: string, user_id?: string | null }>
  initialComments: any[]
  initialSends: Array<{ user_id: string, user_name: string | null, date: string, attempt_type: 'flashed'|'sent', fa?: boolean }>
  initialIsAdmin: boolean
  initialIsRouteSetter: boolean
  currentUserId: string | null
  openLog: boolean
}

export default function ClimbDetailClient({
  climbId,
  initialClimb,
  initialPhotos,
  initialVideos,
  ...
  initialSends,
  initialIsAdmin,
  initialIsRouteSetter,
  currentUserId,
  openLog
}: ClimbDetailClientProps) {
  const focusTick = useFocusTick(250)
  const [climb, setClimb] = useState<Climb | null>(initialClimb)
  const [photos, setPhotos] = useState(initialPhotos)
  const [videos, setVideos] = useState(initialVideos)
  const [comments, setComments] = useState(initialComments)
  const [sends, setSends] = useState(initialSends)
  const [isAdmin, setIsAdmin] = useState(initialIsAdmin)
  const [isRouteSetter, setIsRouteSetter] = useState(initialIsRouteSetter)
  ...
  async function refreshData() {
    try {
      ...
      // Refresh admin status if we have a gym
      if (climb?.gym?.id) {
        const { data: adminData } = await supabase.rpc('is_gym_admin', { gid: climb.gym.id })
        setIsAdmin(!!adminData)
      }

      // Refresh route setter status
      if (currentUserId) {
  ...
  async function addComment(body: string, parent_id?: string, is_setter?: boolean) {
    ...
    const { data, error } = await supabase
      .from('climb_comments')
      .insert({ climb_id: climbId, user_id: uid, body, parent_id: parent_id || null, is_setter: !!is_setter })
        ...
        <VideoAddForm
          onAdd={async (url) => {
            ...
            try {
              ...
              const { data, error } = await supabase.from('climb_videos').insert({ climb_id: climbId, url, user_id: uid }).select('id,url,user_id').single()
