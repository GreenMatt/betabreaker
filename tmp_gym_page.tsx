...
export default function GymDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const [gRes, cRes, aRes, sRes] = await Promise.all([
        ...
        supabase.from('climbs').select('id,name,grade,type,location,setter,color,dyno,section_id,section:gym_sections(name)').eq('gym_id', gid).order('created_at', { ascending: false }).limit(30),
  ...
  const [form, setForm] = useState({ name: '', type: 'boulder' as 'boulder' | 'top_rope' | 'lead', grade: 5, location: '', setter: '', color: 'blue', dyno: false, section_id: '' })
  ...
  async function createClimb(e: React.FormEvent) {
    ...
    setForm({ name: '', type: 'boulder', grade: 5, location: '', setter: '', color: 'blue', dyno: false, section_id: '' })
    const { data } = await supabase.from('climbs').select('id,name,grade,type,location,setter,color,dyno,section_id,section:gym_sections(name)').eq('gym_id', gid).order('created_at', { ascending: false })
    setClimbs(data || [])
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <Link href="/gym" className="text-sm text-base-subtext">â† All Gyms</Link>
      </div>
      ...
      {gym && (
        <div className="card">
          ...
          {!isAdmin && (
            <div className="mt-3">
              <button className="btn-primary" onClick={claimAdmin} disabled={claiming}> {claiming ? 'Claimingâ€¦' : 'Claim Admin (if unclaimed)'} </button>
            </div>
          )}
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {climbs.map(c => {
            ...
            return (
              <div key={c.id} className="group relative rounded-xl overflow-hidden border border-white/5 bg-base-panel shadow">
                ...
                <div className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition">
                  <button className="bg-neon-purple text-white rounded-md px-3 py-1 text-xs" onClick={() => quickLog(c.id)}>Log Send</button>
                </div>
            <div className="p-3">
              ...
              <div className="mt-1 text-xs text-base-subtext">{c.type} â€¢ Grade {c.grade ?? '-'} {c.section?.name ? `â€¢ ${c.section.name}` : c.location ? `â€¢ ${c.location}` : ''}</div>
            </div>
            {isAdmin && (
              <div className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition">
                <button className="bg-white/10 hover:bg-white/20 rounded-md px-2 py-1 text-xs" onClick={() => setEditing({ open: true, climb: c, form: { name: c.name, type: c.type, grade: c.grade ?? 5, setter: c.setter ?? '', section_id: c.section_id ?? '', location: c.location ?? '', color: c.color ?? 'blue', dyno: !!c.dyno } })}>Edit</button>
              </div>
            )}
            </div>
          )
      ...
      <div className="card mt-4">
        ...
        {activityLoading && <div className="text-base-subtext">Loadingâ€¦</div>}
          ...
          {activity.map((a) => (
            <li key={a.id} className="py-3">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm text-base-subtext">{new Date(a.created_at).toLocaleString()}</div>
                  <div className="font-semibold">{a.user_name || 'Climber'} {a.attempt_type} {a.climb_name}</div>
                  <div className="text-xs text-base-subtext">{a.type} â€¢ Grade {a.grade ?? '-'} â€¢ {a.gym_name}</div>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <button className={`rounded-md px-3 py-1 ${a.bumped ? 'bg-neon-purple text-white' : 'bg-white/10 hover:bg-white/20'}`} onClick={async () => {
                    const { data: u } = await supabase.auth.getUser(); const me = u.user?.id; if (!me) return alert('Sign in');
                    if (!a.bumped) {
                      const { error } = await supabase.from('bumps').insert({ log_id: a.id, user_id: me }); if (!error) { a.bumped = true; a.bump_count = (a.bump_count || 0) + 1; setActivity([...activity]) }
                    } else {
                      const { error } = await supabase.from('bumps').delete().eq('log_id', a.id).eq('user_id', me); if (!error) { a.bumped = false; a.bump_count = Math.max(0, (a.bump_count || 0) - 1); setActivity([...activity]) }
                    }
                  }}>ðŸ‘Š {a.bump_count || 0}</button>
                </div>
              </div>
